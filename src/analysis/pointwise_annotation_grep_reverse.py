...
fo_scored = open(args.generation_file + ".pointwise_grep_reverse", "w")
    ...
    for start_index in range(len(prompt_plus_generation)):
        ...
        for end_index in range(latest_end_index, len(prompt_plus_generation) + 1):
            ...
            if not subpart_in_training:
                ...
                skip_to_start_index = end_index - 1
                for i in range(end_index):
                    candidate_subpart = " ".join(prompt_plus_generation[skip_to_start_index:end_index])
                    bash_command = 'LC_ALL=C grep "' + candidate_subpart + '" ' + args.training_file + ' -m 1 -F -q'
